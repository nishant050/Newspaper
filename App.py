...
def create_driver():
    ...
    # Corrected path for Streamlit Cloud deployment
...
def find_all_newspapers(target_date, status_log):
    ...
    found_links = {}
    # Using a ThreadPoolExecutor to run scraping tasks in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(NEWSPAPERS_TO_SCRAPE)) as executor:
        # Create a future for each newspaper scraping task
        ...
        for future in concurrent.futures.as_completed(future_to_newspaper):
            try:
                ...
                status_log.write(f"❌ Error scraping a newspaper: {e}")
    return found_links
...
# Caching logic: Only scrape if the date has changed.
if st.session_state.last_scrape_date != today:
    with st.status("Finding all latest newspapers...", expanded=True) as status:
        ...
        if not any(found_today.values()):
             status.write("No papers for today. Checking yesterday...")
...
# --- Display Logic ---
# This part runs every time, displaying the cached results.
if not st.session_state.found_links or not any(st.session_state.found_links.values()):
    ...
    # Sort the found papers alphabetically by name for a consistent order
    for name, link in sorted(st.session_state.found_links.items()):
        if link:
            try:
                ...
                # If a link is not a valid Google Drive link, just skip it.
                continue
