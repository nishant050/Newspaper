...
def send_email(recipient_email, links_dict, paper_date):
    ...
    try:
        ...
        html_body = f"<html><body><h2>Here are your e-paper links for today:</h2><ul>"
...
if not st.session_state.found_links or not any(st.session_state.found_links.values()):
    ...
    if available_papers_urls:
        ...
        email_input = st.text_input("Enter your email address:", value=default_email)
        
        if st.button("Email Today's Links", use_container_width=True):
            if email_input:
